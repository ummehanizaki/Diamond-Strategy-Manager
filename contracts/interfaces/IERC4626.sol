...
interface IERC4626 {
    ...
    function mint(
        address receiver,
        uint256 assets
    ) external returns (uint256 shares); // Deposits assets and mints shares

    function burn(
        uint256 shares,
        address receiver
    ) external returns (uint256 assets); // Burns shares and withdraws assets

    function deposit(
        uint256 assets,
        address receiver
    ) external returns (uint256 shares); // Deposits assets with optional receiver (ERC20-like)

    function withdraw(
        uint256 assets,
        address receiver,
        address owner
    ) external returns (uint256 shares); // Withdraws assets with optional receiver and owner

    function redeem(
        uint256 shares,
        address receiver,
        address owner
    ) external returns (uint256 assets); // Redeems shares for assets with optional receiver and owner

    // Preview functions (optional)
    function previewDeposit(
        uint256 assets
    ) external view returns (uint256 shares); // Simulates deposit to see minted shares

    function previewMint(uint256 shares) external view returns (uint256 assets); // Simulates mint to see required assets

    function previewWithdraw(
        uint256 shares
    ) external view returns (uint256 assets); // Simulates withdrawal to see received assets

    function previewRedeem(
        uint256 shares
    ) external view returns (uint256 assets); // Simulates redeem to see received assets
